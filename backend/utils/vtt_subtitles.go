...
func buildVttWordSubtitles(b *strings.Builder, t *ent.Transcription, c SubtitleConfig) {
	cueCounter := 1
	...
	for _, segment := range t.Result.Segments {
		for _, word := range segment.Words {
			...
			endTime = word.End
			lastWordHadPunctuation = containsPunctuation(word.Word)
		}
	}
...
func addVttSpeakerStyle(b *strings.Builder, t *ent.Transcription, customColors map[string]string) {
	b.WriteString("STYLE\n")
