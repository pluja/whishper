...
func GenerateSubsASS(t *ent.Transcription, c SubtitleConfig) string {
...
func buildAssWordSubtitles(b *strings.Builder, t *ent.Transcription, c SubtitleConfig) {
	...
	if len(cueWords) > 0 {
		endTime = t.Result.Segments[len(t.Result.Segments)-1].Words[len(t.Result.Segments[len(t.Result.Segments)-1].Words)-1].End
...
func addAssSpeakersStyle(b *strings.Builder, t *ent.Transcription, customColors map[string]string) {
	speakerStyles := make(map[string]string)
	for _, segment := range t.Result.Segments {
		for _, word := range segment.Words {
			if _, ok := speakerStyles[word.Speaker]; !ok && word.Speaker != "" {
				color, _ := slice.Random(assColors)
